...
class Problem:
    ...
    def generate(self, model: str):
        ...
        prompt = (
            f"Solve the following problem in Python and make the"
    ...
    def generate_tags(self) -> str:
        ...
        prompt = (
            f"Create two tags (every tag has to consist of exactly one "
        ...
        try:
            response = MODELS["gpt3"].invoke(prompt)
    ...
    def run(self, language: str = "py", model: Optional[str] = None):
        ...
        if language == "py":
            ...
            result = self.__execute(
                ["python", f"{prefix}solution.py"], capture_output=True
            )
    ...
    def solution_files(self):
        return list(map(lambda x: x.name, self.problem_dir.glob("solution*")))

    def readme_files(self):
        return list(map(lambda x: x.name, self.problem_dir.glob("readme*")))
    ...
    def __open_in_vim(self, language: str, model: str = ""):
        ...
        prefix = self.__model_to_prefix(model)
        self.__execute(
            ["vim", "-o", f"{prefix}solution.{language}", "readme.md"], check=False
        )
    ...
    def _meta_to_pandas_row(problem_dir: Path):
        ...
        for solution, meta in metadata.items():
            ...
            _solution_file = solution_file.format(
                prefix=prefix, lang=language, problem_dir=problem_dir
            )
            if "wall" in meta:
                row[solution] = (
...
def render_template(template: str, dst: Path, params: dict, overwrite: bool = False):
